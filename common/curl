...
cURL supports various protocols like FILE,HTTP,HTTPS,IMAP,IMAPS,LDAP,DICT,LDAPS,TELNET,FTP,FTPS,GOPHER,RTMP,RTSP,SCP,SFTP,POP3,POP3S,SMBS,SMTP,SMTPS,and TFTP
...
Using "curl -C -", we can continue the download from where it left off earlier(if the download was stopped at 20.1%, now the download continues from 20.1%)
...
Sometimes, websites will require a username and password to view the content(can be done with .htaccess file). With the help of -u option, we can pass those credentials from cURL to the web server as shown below
