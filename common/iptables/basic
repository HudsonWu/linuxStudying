...
1) Filter table
Filter is default table for iptables. So, if you don't define you own table, you'll be using filter table. Iptables's filter table has the following built-in chains: INPUT chain, OUTPUT chain, FORWARD chain
2) NAT table
...
4) Raw table
Raw table has the following built-in chains: PREROUTING chain, OUTPUT chain
...
Target Values: ACCEPT, firewall will accept the packet; DROP, firewall will drop the packet; QUEUE, firewall will pass the packet to the userspace; RETURN, firewall will stop executing the next set of rules in the current chain for this packet, the control will be returned to the calling chain
If you do 'iptables -t [table] --list', you'll see all the avaliable firewall rules of the table, if you don't specify the -t option, it will display the default filter table
iptables -t nat --list
...
4. Policy Chain Default Behavior
what do you want iptables to do if the connection doesn't match any existing rules?
1) To see what your policy chains are currently configured to do with unmatched traffic, run the 'iptables -L | grep policy' command
2) To accept connections by default
...
3. Connection-specific Responses
...
4. Allowing or Blocking Specific Connections
...
3) Connections to a specific port
iptables -A INPUT -p tcp --dport ssh -s 10.10.10.10 -j DROP
iptables -A INPUT -p tcp --dport ssh -j DROP

5. Connection States
What is you only want SSH coming into your system to be allowed, won't adding a rule to the output chain also allow outgoing SSH attempts?
That's where connection states come in, which give you the capability you'd need to allow two way communication but only allow one way connections to be established.
