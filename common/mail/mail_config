...
邮件发送方式：
echo "这是一个测试腾讯企业邮箱调用测试邮件"|mail -s "Tencent 企业邮件" zhangluya@zhangluya.com
...
邮件发送方式：
echo "这是一个测试163企业邮箱调用测试邮件"|mail -s "163 企业邮件" zhangluya@zhangluya.com

二：因为阿里云封禁了默认的25端口导致(1)、(2)配置方式不可用，这里可以使用smtps ssl加密替代
