# 更广泛的探索

+ [结构化查询语言和关系型数据库](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/rdbms)
    + [sql语句学习](https://github.com/HudsonWu/linuxStudying/blob/master/less-is-more/rdbms/syntax.md)
    + [mysql数据库](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/rdbms/mysql)
    + [postgresql数据库](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/rdbms/psql)
+ [MongoDB, 基于分布式文件存储的数据库](https://github.com/HudsonWu/linuxStudying/blob/master/less-is-more/mongo.md)
+ [windows系统](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/windows)
    + [bat, 批处理文件](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/windows/bat)
    + [reg, 添加和删除注册表](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/windows/reg)
    + [win7镜像](https://github.com/HudsonWu/linuxStudying/blob/master/less-is-more/windows/win7_images.md)
+ [安全相关](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/security)
+ [折腾过程中的一些记录](https://github.com/HudsonWu/linuxStudying/tree/master/less-is-more/custom)
